...
function providecrt(req, res) {	  
	if (req.body.crtName) {		
		var query = {crtName : req.body.crtName};
		var court = {crtName: req.body.crtName , crtAddr: req.body.crtAddr, crtInfo: req.body.crtInfo};	
		court['verify'] = false;
		
		if (req.files.file) {
			cloudinary.uploader.upload(req.files.file.path, function(result) {
				court['CourtImg'] = result; 
				dbUpdate(courtCollection, query, court, updateCallback);
			});
		}
		else {
			dbUpdate(courtCollection, query, court, updateCallback);	
...
function court(req, res) {	 	
  var query = {};
  var fields = {_id : 0 , crtName : 1 ,crtAddr : 1 , crtInfo : 1, "CourtImg.url" : 1};
  var sortParam = {};
  dbfind(courtCollection, query, fields, sortParam, function(court) {	
  	     console.log(court);	
         if (court) {
	         res.render('court', {
